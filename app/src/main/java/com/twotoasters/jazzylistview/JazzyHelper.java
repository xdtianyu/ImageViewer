...
import org.xdty.imageviewer2.view.adapter.GridAdapter.ViewHolder;
...
public class JazzyHelper implements AbsListView.OnScrollListener {
    ...
    private void doJazzinessImpl(final View item, final int position, final int scrollDirection) {
        final int direction = scrollDirection > 0 ? 1 : -1;
        ((ViewHolder)item.getTag()).jazzyEffect = mTransitionEffect;
        ((ViewHolder)item.getTag()).direction = direction;
    }
